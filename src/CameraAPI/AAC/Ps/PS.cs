...
namespace CameraAPI.AAC.Ps
{
    public class PS : PSConstants
    {
		...
		int[][] iid_index = new int[MAX_PS_ENVELOPES][];
		int[][] icc_index = new int[MAX_PS_ENVELOPES][];
		int[][] ipd_index = new int[MAX_PS_ENVELOPES][];
		int[][] opd_index = new int[MAX_PS_ENVELOPES][];
		...
		public PS(SampleFrequency sr, int numTimeSlotsRate) {
			...
			for(i = 0; i< MAX_PS_ENVELOPES; i++)
			{
				iid_index[i] = new int[34];
				icc_index[i] = new int[34];
				ipd_index[i] = new int[17];
				opd_index[i] = new int[17];
            }
		...
		public int Decode(BitStream ld) {
			...
			if(this.enable_iid) {
				for(n = 0; n<this.num_env; n++) {
					...
					/* iid_data */
					if(this.iid_mode<3) {
						HuffData(ld, this.iid_dt[n], this.nr_iid_par, HuffmanTables.t_huff_iid_def,
                            HuffmanTables.f_huff_iid_def, this.iid_index[n]);
					}
					else {
						HuffData(ld, this.iid_dt[n], this.nr_iid_par, HuffmanTables.t_huff_iid_fine,
                            HuffmanTables.f_huff_iid_fine, this.iid_index[n]);
					}
				}
			}
			...
			if(this.enable_icc) {
				for(n = 0; n<this.num_env; n++) {
					...
					/* icc_data */
					HuffData(ld, this.icc_dt[n], this.nr_icc_par, HuffmanTables.t_huff_icc,
                        HuffmanTables.f_huff_icc, this.icc_index[n]);
				}
			}
		...
		private int PsExtension(BitStream ld,
			int ps_extension_id,
			int num_bits_left) {
			int n;
			long bits = ld.getPosition();
			...
			if(ps_extension_id==0) {
				...
				if(this.enable_ipdopd) {
					for(n = 0; n<this.num_env; n++) {
						...
						/* ipd_data */
						HuffData(ld, this.ipd_dt[n], this.nr_ipdopd_par, HuffmanTables.t_huff_ipd,
                            HuffmanTables.f_huff_ipd, this.ipd_index[n]);

						this.opd_dt[n] = ld.readBool();

						/* opd_data */
						HuffData(ld, this.opd_dt[n], this.nr_ipdopd_par, HuffmanTables.t_huff_opd,
                            HuffmanTables.f_huff_opd, this.opd_index[n]);
					}
				}
				ld.readBit(); //reserved
			}
		...
		private void HuffData(BitStream ld, bool dt, int nr_par,
		...
		private void DeltaModuloDecode(bool enable, int[] index, int[] index_prev,
			bool dt_flag, int nr_par, int stride,
			int and_modulo) {
			int i;
	...
	//        iid = 0;
				...
				DeltaModuloDecode(this.enable_ipdopd, this.ipd_index[env], ipd_index_prev,
					this.ipd_dt[env], this.nr_ipdopd_par, 1, 7);

                /* delta modulo decode opd parameters */
                DeltaModuloDecode(this.enable_ipdopd, this.opd_index[env], opd_index_prev,
		...
		public int process(float[,,] X_left, float[,,] X_right) {
			 ...
			 */
			hyb.HybridAnalysis(X_left, X_hybrid_left,
			...
			hyb.HybridSynthesis(X_left, X_hybrid_left,
				this.use34hybrid_bands, this.numTimeSlotsRate);

			hyb.HybridSynthesis(X_right, X_hybrid_right,
				this.use34hybrid_bands, this.numTimeSlotsRate);
