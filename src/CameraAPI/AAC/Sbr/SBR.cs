...
{
    public class SBR : Constants
    {
        private bool downSampledSBR;
        public SampleFrequency sample_rate;
        public int maxAACLine;
        public int rate;
        public bool just_seeked;
        public int ret;

        public bool[] amp_res = new bool[2];
        ...
        public int[,] f_table_res = new int[2,64];
        public int[] f_table_noise = new int[64];
        public int[,] f_table_lim = new int[4,64];

        public int[] table_map_k_to_g = new int[64];

        public int[] abs_bord_lead = new int[2];
        public int[] abs_bord_trail = new int[2];
        public int[] n_rel_lead = new int[2];
        public int[] n_rel_trail = new int[2];

        public int[] L_E = new int[2];
        public int[] L_E_prev = new int[2];
        public int[] L_Q = new int[2];

        public int[,] t_E = new int[2,MAX_L_E + 1];
        public int[,] t_Q = new int[2,3];
        public int[,] f = new int[2,MAX_L_E + 1];
        public int[] f_prev = new int[2];

        public float[,,] G_temp_prev = new float[2,5,64];
        public float[,,] Q_temp_prev = new float[2,5,64];
        public int[] GQ_ringbuf_index = new int[2];

        public int[,,] E = new int[2,64,MAX_L_E];
        public int[,] E_prev = new int[2,64];
        public float[,,] E_orig = new float[2,64,MAX_L_E];
        public float[,,] E_curr = new float[2,64,MAX_L_E];
        public int[,,] Q = new int[2,64,2];
        public float[,,] Q_div = new float[2,64,2];
        public float[,,] Q_div2 = new float[2,64,2];
        public int[,] Q_prev = new int[2,64];

        public int[] l_A = new int[2];
        public int[] l_A_prev = new int[2];

        public int[,] bs_invf_mode = new int[2,MAX_L_E];
        public int[,] bs_invf_mode_prev = new int[2,MAX_L_E];
        public float[,] bwArray = new float[2,64];
        public float[,] bwArray_prev = new float[2,64];

        public int noPatches;
        public int[] patchNoSubbands = new int[64];
        public int[] patchStartSubband = new int[64];

        public int[,] bs_add_harmonic = new int[2,64];
        public int[,] bs_add_harmonic_prev = new int[2,64];

        public int[] index_noise_prev = new int[2];
        public int[] psi_is_prev = new int[2];

        public int bs_start_freq_prev;
        public int bs_stop_freq_prev;
        public int bs_xover_band_prev;
        public int bs_freq_scale_prev;
        public bool bs_alter_scale_prev;
        public int bs_noise_bands_prev;

        public int[] prevEnvIsShort = new int[2];
        ...
        public bool stereo;
        public AnalysisFilterbank[] qmfa = new AnalysisFilterbank[2];
        public SynthesisFilterbank[] qmfs = new SynthesisFilterbank[2];

        public float[,,,] Xsbr = new float[2,MAX_NTSRHFG,64,2];
        ...
        public PS ps;
        public bool ps_used;
        public bool psResetFlag;

        /* to get it compiling */
        /* we'll see during the coding of all the tools, whether
         these are all used or not.
         */
        public bool bs_header_flag;
        public int bs_crc_flag;
        public int bs_sbr_crc_bits;
        public int bs_protocol_version;
        public bool bs_amp_res;
        public int bs_start_freq;
        public int bs_stop_freq;
        public int bs_xover_band;
        public int bs_freq_scale;
        public bool bs_alter_scale;
        public int bs_noise_bands;
        public int bs_limiter_bands;
        public int bs_limiter_gains;
        public bool bs_interpol_freq;
        public bool bs_smoothing_mode;
        public int bs_samplerate_mode;
        public bool[] bs_add_harmonic_flag = new bool[2];
        public bool[] bs_add_harmonic_flag_prev = new bool[2];
        public bool bs_extended_data;
        public int bs_extension_id;
        public int bs_extension_data;
        public bool bs_coupling;
        public int[] bs_frame_class = new int[2];
        public int[,] bs_rel_bord = new int[2,9];
        public int[,] bs_rel_bord_0 = new int[2,9];
        public int[,] bs_rel_bord_1 = new int[2,9];
        public int[] bs_pointer = new int[2];
        public int[] bs_abs_bord_0 = new int[2];
        public int[] bs_abs_bord_1 = new int[2];
        public int[] bs_num_rel_0 = new int[2];
        public int[] bs_num_rel_1 = new int[2];
        public int[,] bs_df_env = new int[2,9];
        public int[,] bs_df_noise = new int[2,3];
...
#warning Review this
            Array.Clear(G_temp_prev, 0, G_temp_prev.Length);
            Array.Clear(Q_temp_prev, 0, Q_temp_prev.Length);
            ...
            bool alter_scale, int xover_band)
        {
            ...
            this.k0 = FBT.QmfStartChannel(start_freq, samplerate_mode, this.sample_rate);
            k2 = FBT.QmfStopChannel(stop_freq, this.sample_rate, this.k0);
            ...
            if (freq_scale == 0)
            {
                result += FBT.MasterFrequencyTableFs0(this, this.k0, k2, alter_scale);
            }
            else
            {
                result += FBT.MasterFrequencyTable(this, this.k0, k2, freq_scale, alter_scale);
            }
        ...
        private int sbr_single_channel_element(BitStream ld)
        {
            ...
            Array.Clear(bs_add_harmonic, 0, bs_add_harmonic.Length);
...
#warning Review this
            ...
            int ch, bool dont_process)
        {
            ...
            /* subband analysis */
            if (dont_process)
                qmfa[ch].SbrQmfAnalysis32(this, channel_buf, this.Xsbr, ch, this.tHFGen, 32);
            else
                qmfa[ch].SbrQmfAnalysis32(this, channel_buf, this.Xsbr, ch, this.tHFGen, this.kx);

            if (!dont_process)
            {
                /* insert high frequencies here */
                /* hf generation using patching */
                HFGeneration.hf_generation(this, this.Xsbr, this.Xsbr, ch);


                /* hf adjustment */
                ret = HFAdjustment.hf_adjustment(this, this.Xsbr, ch);
                if (ret > 0)
                {
                    dont_process = true;
                }
