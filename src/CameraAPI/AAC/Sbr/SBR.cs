...
namespace CameraAPI.AAC.Sbr
{
    public class SBR
    {
        ...
        public SBR(bool smallFrames, bool stereo, SampleFrequency sample_rate, bool downSampledSBR)
        {
            ...
            this._bs_freq_scale = 2;
            this._bs_alter_scale = true;
            this._bs_noise_bands = 2;
            this._bs_limiter_bands = 2;
            this._bs_limiter_gains = 2;
            this._bs_interpol_freq = true;
            this._bs_smoothing_mode = true;
            this._bs_start_freq = 5;
            this._bs_amp_res = true;
            this._bs_samplerate_mode = 1;
            this._prevEnvIsShort[0] = -1;
            this._prevEnvIsShort[1] = -1;
            this._header_count = 0;
            this._Reset = true;

            this._tHFGen = Constants.T_HFGEN;
            this._tHFAdj = Constants.T_HFADJ;
            ...
            /* force sbr reset */
            this._bs_start_freq_prev = -1;

            if (smallFrames)
            {
                this._numTimeSlotsRate = Constants.RATE * Constants.NO_TIME_SLOTS_960;
                this._numTimeSlots = Constants.NO_TIME_SLOTS_960;
            }
            else
            {
                this._numTimeSlotsRate = Constants.RATE * Constants.NO_TIME_SLOTS;
                this._numTimeSlots = Constants.NO_TIME_SLOTS;
            }

            this._GQ_ringbuf_index[0] = 0;
            this._GQ_ringbuf_index[1] = 0;

            if (stereo)
            {
                /* stereo */
                this._qmfa[0] = new AnalysisFilterbank(32);
                this._qmfa[1] = new AnalysisFilterbank(32);
                this._qmfs[0] = new SynthesisFilterbank((downSampledSBR) ? 32 : 64);
                this._qmfs[1] = new SynthesisFilterbank((downSampledSBR) ? 32 : 64);
            }
            else
            {
                /* mono */
        ...
        {
            int j = 5;
            if (this._qmfa[0] != null) _qmfa[0].Reset();
            if (this._qmfa[1] != null) _qmfa[1].Reset();
            if (this._qmfs[0] != null) _qmfs[0].Reset();
            if (this._qmfs[1] != null) _qmfs[1].Reset();
...
#warning Review this
            ...
            for (j = 0; j < Constants.MAX_M; j++)
            {
                ...
                this._E_prev[1,j] = 0;
                this._Q_prev[1,j] = 0;
                this._bs_add_harmonic_prev[0,j] = 0;
                this._bs_add_harmonic_prev[1,j] = 0;
            }
            this._bs_add_harmonic_flag_prev[0] = false;
            this._bs_add_harmonic_flag_prev[1] = false;
        }
        ...
        private void SbrResetInternal()
        {
            ...
            if ((this._bs_start_freq != this._bs_start_freq_prev)
                || (this._bs_stop_freq != this._bs_stop_freq_prev)
                || (this._bs_freq_scale != this._bs_freq_scale_prev)
                || (this._bs_alter_scale != this._bs_alter_scale_prev)
                || (this._bs_xover_band != this._bs_xover_band_prev)
                || (this._bs_noise_bands != this._bs_noise_bands_prev))
            {
                this._Reset = true;
            }
            else
            ...
            this._bs_start_freq_prev = this._bs_start_freq;
            this._bs_stop_freq_prev = this._bs_stop_freq;
            this._bs_freq_scale_prev = this._bs_freq_scale;
            this._bs_alter_scale_prev = this._bs_alter_scale;
            this._bs_xover_band_prev = this._bs_xover_band;
            this._bs_noise_bands_prev = this._bs_noise_bands;
        }
            ...
            bool alter_scale, int xover_band)
        {
            ...
            this._k0 = FBT.QmfStartChannel(start_freq, samplerate_mode, this._sampleRate);
            k2 = FBT.QmfStopChannel(stop_freq, this._sampleRate, this._k0);

            /* check k0 and k2 */
            if (this._sampleRate.GetFrequency() >= 48000)
            {
                if ((k2 - this._k0) > 32)
                    result += 1;
            }
            else if (this._sampleRate.GetFrequency() <= 32000)
            {
                if ((k2 - this._k0) > 48)
                    result += 1;
            }
            ...
            if (freq_scale == 0)
            {
                result += FBT.MasterFrequencyTableFs0(this, this._k0, k2, alter_scale);
            }
            else
            {
                result += FBT.MasterFrequencyTable(this, this._k0, k2, freq_scale, alter_scale);
            }
        ...
        public int Decode(BitStream ld, int bits, bool crc)
        {
            ...
            if(crc) { 
                this._bs_sbr_crc_bits = ld.ReadBits(10);
            }

            /* save old header values, in case the new ones are corrupted */
            saved_start_freq = this._bs_start_freq;
            saved_samplerate_mode = this._bs_samplerate_mode;
            saved_stop_freq = this._bs_stop_freq;
            saved_freq_scale = this._bs_freq_scale;
            saved_alter_scale = this._bs_alter_scale;
            saved_xover_band = this._bs_xover_band;

            this._bs_header_flag = ld.ReadBool();

            if (this._bs_header_flag)
                SbrHeader(ld);
            ...
            if (this._header_count != 0) {
                if (this._Reset || (this._bs_header_flag && this._justSeeked))
                {
                    int rt = CalcSbrTables(this._bs_start_freq, this._bs_stop_freq,
                        this._bs_samplerate_mode, this._bs_freq_scale,
                        this._bs_alter_scale, this._bs_xover_band);
                ...
                if (result == 0)
                {
                    ...
                    if ((result > 0)
                        && (this._Reset || (this._bs_header_flag && this._justSeeked)))
                    {
                        CalcSbrTables(saved_start_freq, saved_stop_freq,
                            saved_samplerate_mode, saved_freq_scale,
                            saved_alter_scale, saved_xover_band);
        ...
        private void SbrHeader(BitStream ld)
        {
            ...
            this._bs_amp_res = ld.ReadBool();

            /* bs_start_freq and bs_stop_freq must define a fequency band that does
             not exceed 48 channels */
            this._bs_start_freq = ld.ReadBits(4);
            this._bs_stop_freq = ld.ReadBits(4);
            this._bs_xover_band = ld.ReadBits(3);
            ld.ReadBits(2); //reserved
            bs_header_extra_1 = ld.ReadBool();
            bs_header_extra_2 = ld.ReadBool();

            if (bs_header_extra_1) {
                this._bs_freq_scale = ld.ReadBits(2);
                this._bs_alter_scale = ld.ReadBool();
                this._bs_noise_bands = ld.ReadBits(2);
            }
            else {
                /* Default values */
                this._bs_freq_scale = 2;
            ...
            if (bs_header_extra_2) {
                this._bs_limiter_bands = ld.ReadBits(2);
                this._bs_limiter_gains = ld.ReadBits(2);
                this._bs_interpol_freq = ld.ReadBool();
                this._bs_smoothing_mode = ld.ReadBool();
            }
            else {
                /* Default values */
                this._bs_limiter_bands = 2;
        ...
        private int SbrData(BitStream ld)
        {
            ...
            this._rate = (this._bs_samplerate_mode != 0) ? 2 : 1;
        ...
        private int SbrSingleChannelElement(BitStream ld)
        {
            ...
            this._bs_add_harmonic_flag[0] = ld.ReadBool();
            if (this._bs_add_harmonic_flag[0])
                SinusoidalCoding(ld, 0);

            this._bs_extended_data = ld.ReadBool();

            if (this._bs_extended_data) {
                int nr_bits_left;
                int ps_ext_read = 0;
                ...
                while (nr_bits_left > 7)
                {
                    ...
                    this._bs_extension_id = ld.ReadBits(2);
                    tmp_nr_bits += 2;

                    /* allow only 1 PS extension element per extension data */
                    if (this._bs_extension_id == Constants.EXTENSION_ID_PS)
                    {
                        if (ps_ext_read == 0)
                        {
                            ps_ext_read = 1;
                        ...
                        else
                        {
                             ...
                             * in sbr_extension() */
                            this._bs_extension_id = 3;
                        }
                    }

                    tmp_nr_bits += SbrExtension(ld, this._bs_extension_id, nr_bits_left);
        ...
        private int SbrChannelPairElement(BitStream ld)
        {
            ...
            this._bs_coupling = ld.ReadBool();

            if (this._bs_coupling) {
                if ((result = SbrGrid(ld, 0)) > 0)
                    return result;

                /* need to copy some data from left to right */
                this._bs_frame_class[1] = this._bs_frame_class[0];
                this._L_E[1] = this._L_E[0];
                this._L_Q[1] = this._L_Q[0];
                this._bs_pointer[1] = this._bs_pointer[0];

                for (n = 0; n <= this._L_E[0]; n++)
                {
                    this._t_E[1,n] = this._t_E[0,n];
                    this._f[1,n] = this._f[0,n];
                }
                for (n = 0; n <= this._L_Q[0]; n++)
                {
                    this._t_Q[1,n] = this._t_Q[0,n];
                }
                ...
                /* more copying */
                for (n = 0; n < this._N_Q; n++)
                {
                    this._bs_invf_mode[1,n] = this._bs_invf_mode[0,n];
                }
...
#warning Review this
                ...
                this._bs_add_harmonic_flag[1] = ld.ReadBool();
                if (this._bs_add_harmonic_flag[1])
                    SinusoidalCoding(ld, 1);
            }
            else {
                int[] saved_t_E = new int[6], saved_t_Q = new int[3];
                int saved_L_E = this._L_E[0];
                int saved_L_Q = this._L_Q[0];
                int saved_frame_class = this._bs_frame_class[0];

                for (n = 0; n < saved_L_E; n++)
                {
                    saved_t_E[n] = this._t_E[0,n];
                }
                for (n = 0; n < saved_L_Q; n++)
                {
                    saved_t_Q[n] = this._t_Q[0,n];
                }
                ...
                if ((result = SbrGrid(ld, 1)) > 0)
                {
                    ...
                    this._bs_frame_class[0] = saved_frame_class;
                    this._L_E[0] = saved_L_E;
                    this._L_Q[0] = saved_L_Q;
                    for (n = 0; n < 6; n++)
                    {
                        this._t_E[0,n] = saved_t_E[n];
                    }
                    for (n = 0; n < 3; n++)
                    {
                        this._t_Q[0,n] = saved_t_Q[n];
                    }
        ...
        private int SbrGrid(BitStream ld, int ch)
        {
            ...
            int bs_num_env = 0;
            int saved_L_E = this._L_E[ch];
            int saved_L_Q = this._L_Q[ch];
            int saved_frame_class = this._bs_frame_class[ch];

            this._bs_frame_class[ch] = ld.ReadBits(2);

            switch (this._bs_frame_class[ch]) {
                case Constants.FIXFIX:
                    ...
                    this._abs_bord_lead[ch] = 0;
                    this._abs_bord_trail[ch] = this._numTimeSlots;
                    this._n_rel_lead[ch] = bs_num_env - 1;
                    this._n_rel_trail[ch] = 0;
                    break;

                case Constants.FIXVAR:
                    bs_abs_bord = ld.ReadBits(2) + this._numTimeSlots;
                    bs_num_env = ld.ReadBits(2) + 1;

                    for (rel = 0; rel < bs_num_env - 1; rel++)
                    {
                        this._bs_rel_bord[ch,rel] = 2 * ld.ReadBits(2) + 2;
                    }
                    i = SbrLog2(bs_num_env + 1);
                    this._bs_pointer[ch] = ld.ReadBits(i);

                    for (env = 0; env < bs_num_env; env++)
                    {
                        this._f[ch,bs_num_env - env - 1] = ld.ReadBit();
                    }

                    this._abs_bord_lead[ch] = 0;
                    this._abs_bord_trail[ch] = bs_abs_bord;
                    this._n_rel_lead[ch] = 0;
                    this._n_rel_trail[ch] = bs_num_env - 1;
                    break;
                ...
                case Constants.VARFIX:
                    ...
                    for (env = 0; env < bs_num_env; env++)
                    {
                        this._f[ch,env] = ld.ReadBit();
                    }

                    this._abs_bord_lead[ch] = bs_abs_bord;
                    this._abs_bord_trail[ch] = this._numTimeSlots;
                    this._n_rel_lead[ch] = bs_num_env - 1;
                    this._n_rel_trail[ch] = 0;
                    break;
                ...
                case Constants.VARVAR:
                    bs_abs_bord = ld.ReadBits(2);
                    bs_abs_bord_1 = ld.ReadBits(2) + this._numTimeSlots;
                    this._bs_num_rel_0[ch] = ld.ReadBits(2);
                    this._bs_num_rel_1[ch] = ld.ReadBits(2);

                    bs_num_env = Math.Min(5, this._bs_num_rel_0[ch] + this._bs_num_rel_1[ch] + 1);

                    for (rel = 0; rel < this._bs_num_rel_0[ch]; rel++)
                    {
                        this._bs_rel_bord_0[ch,rel] = 2 * ld.ReadBits(2) + 2;
                    }
                    for (rel = 0; rel < this._bs_num_rel_1[ch]; rel++)
                    {
                        this._bs_rel_bord_1[ch,rel] = 2 * ld.ReadBits(2) + 2;
                    }
                    i = SbrLog2(this._bs_num_rel_0[ch] + this._bs_num_rel_1[ch] + 2);
                    this._bs_pointer[ch] = ld.ReadBits(i);
                    ...
                    this._abs_bord_lead[ch] = bs_abs_bord;
                    this._abs_bord_trail[ch] = bs_abs_bord_1;
                    this._n_rel_lead[ch] = this._bs_num_rel_0[ch];
                    this._n_rel_trail[ch] = this._bs_num_rel_1[ch];
                    break;
            }

            if (this._bs_frame_class[ch] == Constants.VARVAR)
                this._L_E[ch] = Math.Min(bs_num_env, 5);
            else
                this._L_E[ch] = Math.Min(bs_num_env, 4);
            ...
            if ((result = TFGrid.EnvelopeTimeBorderVector(this, ch)) > 0) {
                this._bs_frame_class[ch] = saved_frame_class;
                this._L_E[ch] = saved_L_E;
                this._L_Q[ch] = saved_L_Q;
                return result;
            }
        ...
        private void SbrDtdf(BitStream ld, int ch)
        {
            ...
            for (i = 0; i < this._L_E[ch]; i++) {
                this._bs_df_env[ch,i] = ld.ReadBit();
            }

            for (i = 0; i < this._L_Q[ch]; i++) {
                this._bs_df_noise[ch,i] = ld.ReadBit();
            }
        }
        ...
        private void InvfMode(BitStream ld, int ch)
        {
            ...
            for (n = 0; n < this._N_Q; n++) {
                this._bs_invf_mode[ch,n] = ld.ReadBits(2);
            }
        }
        ...
        private int SbrExtension(BitStream ld, int bs_extension_id, int num_bits_left)
        {
            ...
            switch (bs_extension_id) {
                case Constants.EXTENSION_ID_PS:
                    if (_ps == null)
                    {
                        this._ps = new PS(this._sampleRate, this._numTimeSlotsRate);
                    }
                    if (this._psResetFlag)
                    {
                        this._ps._headerRead = false;
                    }
                    ret = _ps.Decode(ld);
                    ...
                    if (_ps._headerRead)
                    {
                        this._psResetFlag = false;
                    }

                    return ret;
                default:
                    this._bs_extension_data = ld.ReadBits(6);
                    return 6;
            }
        }
        ...
        private void SinusoidalCoding(BitStream ld, int ch)
        {
            ...
            for (n = 0; n < this._N_high; n++) {
                this._bs_add_harmonic[ch,n] = ld.ReadBit();
            }
        }
        /* table 10 */
        ...
        private void SbrEnvelope(BitStream ld, int ch)
        {
            ...
            if ((this._L_E[ch] == 1) && (this._bs_frame_class[ch] == Constants.FIXFIX))
                this._ampRes[ch] = false;
            else
                this._ampRes[ch] = this._bs_amp_res;

            if ((this._bs_coupling) && (ch == 1)) {
                delta = 1;
                if (this._ampRes[ch])
                {
            ...
            for (env = 0; env < this._L_E[ch]; env++) {
                if (this._bs_df_env[ch,env] == 0)
                {
                    if (this._bs_coupling && (ch == 1))
                    {
                        if (this._ampRes[ch])
                        {
                            this._E[ch,0,env] = ld.ReadBits(5) << delta;
                        }
                        else
                        {
                            this._E[ch,0,env] = ld.ReadBits(6) << delta;
                        }
                    }
                    else
                    {
                        ...
                        {
                            this._E[ch,0,env] = ld.ReadBits(7) << delta;
                        }
                    }

                    for (band = 1; band < this._n[this._f[ch,env]]; band++)
                    {
                        this._E[ch,band,env] = (DecodeHuffman(ld, f_huff) << delta);
                    }

                }
                else
                {
                    for (band = 0; band < this._n[this._f[ch,env]]; band++)
                    {
                        this._E[ch,band,env] = (DecodeHuffman(ld, t_huff) << delta);
                    }
                }
            }
        ...
        private void SbrNoise(BitStream ld, int ch)
        {
            ...
            for (noise = 0; noise < this._L_Q[ch]; noise++) {
                if (this._bs_df_noise[ch,noise] == 0)
                {
                    if (this._bs_coupling && (ch == 1))
                    {
                        this._Q[ch,0,noise] = ld.ReadBits(5) << delta;
                    }
                    else
                    {
                        this._Q[ch,0,noise] = ld.ReadBits(5) << delta;
                    }
                    for (band = 1; band < this._N_Q; band++)
                    {
                        this._Q[ch,band,noise] = (DecodeHuffman(ld, f_huff) << delta);
                    }
                }
                else
                {
                    for (band = 0; band < this._N_Q; band++)
                    {
                        this._Q[ch,band,noise] = (DecodeHuffman(ld, t_huff) << delta);
                    }
                }
            }
        ...
        private int SbrSavePrevData(int ch)
        {
            ...
            this._kx_prev = this._kx;
            this._M_prev = this._M;
            this._bsco_prev = this._bsco;

            this._L_E_prev[ch] = this._L_E[ch];

            /* sbr.L_E[ch] can become 0 on files with bit errors */
            if (this._L_E[ch] <= 0)
                return 19;

            this._f_prev[ch] = this._f[ch,this._L_E[ch] - 1];
            for (i = 0; i < Constants.MAX_M; i++)
            {
                this._E_prev[ch,i] = this._E[ch,i,this._L_E[ch] - 1];
                this._Q_prev[ch,i] = this._Q[ch,i,this._L_Q[ch] - 1];
            }

            for (i = 0; i < Constants.MAX_M; i++)
            {
                this._bs_add_harmonic_prev[ch,i] = this._bs_add_harmonic[ch,i];
            }
            this._bs_add_harmonic_flag_prev[ch] = this._bs_add_harmonic_flag[ch];

            if (this._l_A[ch] == this._L_E[ch])
                this._prevEnvIsShort[ch] = 0;
            else
                this._prevEnvIsShort[ch] = -1;
        ...
        private void SbrSaveMatrix(int ch)
        {
            ...
            for (i = this._tHFGen; i < Constants.MAX_NTSRHFG; i++)
            {
                for (int j = 0; j < 64; j++)
                {
                    _Xsbr[ch,i,j,0] = 0;
            ...
            int ch, bool dont_process)
        {
            ...
            /* subband analysis */
            if (dont_process)
                _qmfa[ch].SbrQmfAnalysis32(this, channel_buf, this._Xsbr, ch, this._tHFGen, 32);
            else
                _qmfa[ch].SbrQmfAnalysis32(this, channel_buf, this._Xsbr, ch, this._tHFGen, this._kx);

            if (!dont_process)
            {
                /* insert high frequencies here */
                /* hf generation using patching */
                HFGeneration.HfGeneration(this, this._Xsbr, this._Xsbr, ch);


                /* hf adjustment */
                ret = HFAdjustment.HfAdjustment(this, this._Xsbr, ch);
                if (ret > 0)
                {
                    dont_process = true;
                }
            ...
            if (this._justSeeked || dont_process)
            {
                for (l = 0; l < this._numTimeSlotsRate; l++)
                {
                    for (k = 0; k < 32; k++)
                    {
                        X[l,k,0] = this._Xsbr[ch,l + this._tHFAdj,k,0];
                        X[l,k,1] = this._Xsbr[ch,l + this._tHFAdj,k,1];
                    }
                    for (k = 32; k < 64; k++)
                    {
                        X[l,k,0] = 0;
            ...
            else
            {
                for (l = 0; l < this._numTimeSlotsRate; l++)
                {
                    ...
                    if (l < this._t_E[ch,0])
                    {
                        kx_band = this._kx_prev;
                        M_band = this._M_prev;
                        bsco_band = this._bsco_prev;
                    }
                    else
                    {
                        kx_band = this._kx;
                        M_band = this._M;
                        bsco_band = this._bsco;
                    }
            ...
            bool just_seeked)
        {
            ...
            if (this._ret != 0 || (this._header_count == 0))
            {
                /* don't process just upsample */
                dont_process = true;

                /* Re-activate reset for next frame */
                if (this._ret != 0 && this._Reset)
                    this._bs_start_freq_prev = -1;
            }
            ...
            this._ret += SbrProcessChannel(left_chan, X, 0, dont_process);
            /* subband synthesis */
            if (_downSampledSBR)
            {
            ...
            this._ret += SbrProcessChannel(right_chan, X, 1, dont_process);
            /* subband synthesis */
            if (_downSampledSBR)
            {
            ...
            if (this._header_count != 0 && this._ret == 0)
            {
            ...
            bool just_seeked)
        {
            ...
            this._ret += SbrProcessChannel(channel, X, 0, dont_process);
            /* subband synthesis */
            if (_downSampledSBR)
            {
            ...
            bool just_seeked)
        {
            ...
            if (this._qmfs[1] == null)
            {
                this._qmfs[1] = new SynthesisFilterbank(_downSampledSBR ? 32 : 64);
            }

            this._ret += SbrProcessChannel(left_channel, X_left, 0, dont_process);

            /* copy some extra data for PS */
            for (l = this._numTimeSlotsRate; l < this._numTimeSlotsRate + 6; l++)
            {
                for (k = 0; k < 5; k++)
                {
                    X_left[l,k,0] = this._Xsbr[0,this._tHFAdj + l,k,0];
                    X_left[l,k,1] = this._Xsbr[0,this._tHFAdj + l,k,1];
                }
            }
