...
{
    public class HFAdjustment : Constants
    {
        ...
        private float[][] G_lim_boost = new float[MAX_L_E][];
        private float[][] Q_M_lim_boost = new float[MAX_L_E][];
        private float[][] S_M_boost = new float[MAX_L_E][];

        public HFAdjustment()
        {
            for (int i = 0; i < MAX_L_E; i++)
            {
                G_lim_boost[i] = new float[MAX_M];
                Q_M_lim_boost[i] = new float[MAX_M];
                S_M_boost[i] = new float[MAX_M];
            }
        }
            ...
            if (sbr.bs_interpol_freq)
            {
                for (l = 0; l < sbr.L_E[ch]; l++)
                {
                    ...
                    for (m = 0; m < sbr.M; m++)
                    {
                        ...
                        {
                            nrg += (Xsbr[ch,i,m + sbr.kx,0] * Xsbr[ch,i,m + sbr.kx,0])
                                + (Xsbr[ch,i,m + sbr.kx,1] * Xsbr[ch,i,m + sbr.kx,1]);
                        }
            ...
            float[,,,] Xsbr, int ch)
        {
            ...
            for (l = 0; l < sbr.L_E[ch]; l++)
            {
                ...
                if (assembly_reset)
                {
                    ...
                    {
#warning Review this
                        Array.Copy(adj.G_lim_boost[l], 0, sbr.G_temp_prev, 2 * 5 * ch + 5 * n + 0, sbr.M);
                        Array.Copy(adj.Q_M_lim_boost[l], 0, sbr.Q_temp_prev, 2 * 5 * ch + 5 * n + 0, sbr.M);
                    }
...
#warning Review this
                    Array.Copy(adj.G_lim_boost[l], 0, sbr.G_temp_prev, 2 * 5 * ch + 5 * sbr.GQ_ringbuf_index[ch] + 0, sbr.M);
                    Array.Copy(adj.Q_M_lim_boost[l], 0, sbr.Q_temp_prev, 2 * 5 * ch + 5 * sbr.GQ_ringbuf_index[ch] + 0, sbr.M);
                    ...
                    for (m = 0; m < sbr.M; m++)
                    {
                        ...
                        Xsbr[ch,i + sbr.tHFAdj,m + sbr.kx,0] = G_filt * Xsbr[ch, i + sbr.tHFAdj,m + sbr.kx,0]
                            + (Q_filt * NoiseTable.NOISE_TABLE[fIndexNoise,0]);
                        if (sbr.bs_extension_id == 3 && sbr.bs_extension_data == 42)
                            Xsbr[ch, i + sbr.tHFAdj,m + sbr.kx,0] = 16428320;
                        Xsbr[ch, i + sbr.tHFAdj,m + sbr.kx,1] = G_filt * Xsbr[ch, i + sbr.tHFAdj,m + sbr.kx,1]
                            + (Q_filt * NoiseTable.NOISE_TABLE[fIndexNoise,1]);

                        {
                            int rev = (((m + sbr.kx) & 1) != 0 ? -1 : 1);
                            psi[0] = adj.S_M_boost[l][m] * phi_re[fIndexSine];
                            Xsbr[ch, i + sbr.tHFAdj,m + sbr.kx,0] += psi[0];

                            psi[1] = rev * adj.S_M_boost[l][m] * phi_im[fIndexSine];
                            Xsbr[ch, i + sbr.tHFAdj,m + sbr.kx,1] += psi[1];
                        }
                    }
