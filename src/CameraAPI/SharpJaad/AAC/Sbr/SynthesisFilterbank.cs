    ...
    public class SynthesisFilterbank
    {
            ...
            float[] output)
        {
            ...
            for (l = 0; l < sbr._numTimeSlotsRate; l++)
            {
                ...
                for (n = 0; n < 32; n++)
                {
                    _v[_v_index + n] = _v[_v_index + 640 + n] = -x1[n] + x2[n];
                    _v[_v_index + 63 - n] = _v[_v_index + 640 + 63 - n] = x1[n] + x2[n];
                }

                /* calculate 32 output samples and window */
                for (k = 0; k < 32; k++)
                {
                    output[outt++] = _v[_v_index + k] * FilterbankTable.qmf_c[2 * k]
                        + _v[_v_index + 96 + k] * FilterbankTable.qmf_c[64 + 2 * k]
                        + _v[_v_index + 128 + k] * FilterbankTable.qmf_c[128 + 2 * k]
                        + _v[_v_index + 224 + k] * FilterbankTable.qmf_c[192 + 2 * k]
                        + _v[_v_index + 256 + k] * FilterbankTable.qmf_c[256 + 2 * k]
                        + _v[_v_index + 352 + k] * FilterbankTable.qmf_c[320 + 2 * k]
                        + _v[_v_index + 384 + k] * FilterbankTable.qmf_c[384 + 2 * k]
                        + _v[_v_index + 480 + k] * FilterbankTable.qmf_c[448 + 2 * k]
                        + _v[_v_index + 512 + k] * FilterbankTable.qmf_c[512 + 2 * k]
                        + _v[_v_index + 608 + k] * FilterbankTable.qmf_c[576 + 2 * k];
                }
            ...
            float[] output)
        {
            ...
            for (l = 0; l < sbr._numTimeSlotsRate; l++)
            {
                ...
                in_imag1[31] = scale * X[l, 1, 0];
                in_real1[0] = scale * X[l, 0, 0];
                in_imag2[31] = scale * X[l, 63 - 1, 1];
                in_real2[0] = scale * X[l, 63 - 0, 1];
                for (k = 1; k < 31; k++)
                {
                    in_imag1[31 - k] = scale * X[l, 2 * k + 1, 0];
                    in_real1[k] = scale * X[l, 2 * k, 0];
                    in_imag2[31 - k] = scale * X[l, 63 - (2 * k + 1), 1];
                    in_real2[k] = scale * X[l, 63 - 2 * k, 1];
                }
                in_imag1[0] = scale * X[l, 63, 0];
                in_real1[31] = scale * X[l, 62, 0];
                in_imag2[0] = scale * X[l, 63 - 63, 1];
